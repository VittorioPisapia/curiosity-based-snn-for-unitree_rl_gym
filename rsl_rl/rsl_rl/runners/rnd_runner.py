...
class RndRunner ( SnnRunner ):
    def __init__(self,
        ...
        self.curiosity_cfg = self.policy_cfg["curiosity"]
        self.rnd_cfg = self.curiosity_cfg["rnd"]
        self.device = device
        self.env = env
        ...
        self.use_rnd = self.rnd_cfg.get("use_rnd", True)

        self.alg_cfg["use_rnd"] = self.use_rnd
        self.alg_cfg["rnd_cfg"] = self.rnd_cfg
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        int_rewbuffer = deque(maxlen=100)
        cur_reward_sum = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device)
        cur_int_reward_sum = torch.zeros(self.env.num_envs, dtype=torch.float, device=self.device)
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    if self.use_rnd:
                        ...
                        self.alg.rnd.update_normalization(obs_without_commands)
                        intrinsic_reward = self.alg.rnd.get_intrinsic_reward(obs_without_commands)
                        
                        rewards += intrinsic_reward
                        cur_int_reward_sum += intrinsic_reward
                    ...
                    if self.log_dir is not None:
                        ...
                        if self.use_rnd:
                            int_rewbuffer.extend(cur_int_reward_sum[new_ids][:, 0].cpu().numpy().tolist())
                        lenbuffer.extend(cur_episode_length[new_ids][:, 0].cpu().numpy().tolist())
                        cur_reward_sum[new_ids] = 0
                        cur_int_reward_sum[new_ids] = 0
