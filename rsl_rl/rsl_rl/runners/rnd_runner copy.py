...
class RndRunner ( SnnRunner ):
    def __init__(self,
        ...
        self.beta_max = 0.00005   
        self.lambda_eta = 10.0    
        self.rew_window = 20
        self.ext_rew_history = deque(maxlen=self.rew_window)
        self.intrinsic_weight = self.beta_max 
            ...
            with torch.inference_mode():
                for i in range(self.num_steps_per_env):
                    ...
                    if self.use_rnd:
                        ...
                        mean_intrinsic_reward += (self.intrinsic_weight * intrinsic_rewards.mean()).item()
                ...
                if self.use_rnd and len(ext_rewbuffer) >= 2:
                    current_mean_ext = statistics.mean(ext_rewbuffer)
                    self.ext_rew_history.append(current_mean_ext)

                    if len(self.ext_rew_history) >= 2:
                        denom = abs(self.ext_rew_history[0]) + 1e-5
                        slope = (self.ext_rew_history[-1] - self.ext_rew_history[0]) / denom
                        self.current_slope = max(0, slope)
                        
                        self.intrinsic_weight = (self.beta_max * torch.exp(torch.tensor(-self.lambda_eta * slope))).item()
                        self.alg.rnd_weight = self.intrinsic_weight
    ...
    def log(self, locs, width=80, pad=35):
        ...
        if len(locs['rewbuffer']) > 0:
            ...
            if self.use_rnd:
                ...
                if self.use_rnd:
                    self.writer.add_scalar('RND/intrinsic_weight', self.intrinsic_weight, locs['it'])
                    self.writer.add_scalar('RND/normalized_slope', self.current_slope, locs['it'])
                if locs.get('mean_rnd_loss') is not None:
                    self.writer.add_scalar('Loss/rnd_predictor_loss', locs['mean_rnd_loss'], locs['it'])
